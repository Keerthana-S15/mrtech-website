...
export default function LocationMap({ onLocationSelect }) {
  ...
  const handleSelectSuggestion = (feature) => {
    ...
    setSuggestions([]);
    if (onLocationSelect) {
      onLocationSelect({ lat, lng: lon, address });
    }
  };
  ...
  const handleMapClick = async (newPosition) => {
    ...
    try {
      const res = await fetch(
        `https://photon.komoot.io/reverse?lon=${newPosition[1]}&lat=${newPosition[0]}`
      );
      if (!res.ok) throw new Error("Reverse geocode request failed");
      const data = await res.json();
      const feature = data.features && data.features[0];
      const address = feature ? formatPhotonAddress(feature) : "Selected location";
      setAddressText(address);
      setQuery(address);
      if (onLocationSelect) {
        onLocationSelect({ lat: newPosition[0], lng: newPosition[1], address });
